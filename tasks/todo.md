...
- [x] Restore admin participation in normal playable rooms.
- [x] Add admin-only room deletion plus forced round progression controls.
- [x] Add admin proxy-play controls so admin can act on behalf of the current chooser or current turn player.
- [x] Verify each touched file with `git diff -- <file>`.
- [x] Run the relevant tests and build commands for the admin control update.

Review:
- Restored admin create/join/play access, added admin room deletion, and added server-authoritative admin force-start / force-progress / proxy-turn actions with matching multiplayer UI controls.

- [x] Show each round result in a dedicated modal instead of only inline on the board.
- [x] Keep the result modal visible even after the next round setup starts, until the user closes it.
- [x] Verify each touched file with `git diff -- <file>`.
- [x] Run the relevant build command for the result-modal update.

Review:
- Added a dedicated round-result modal driven by persisted round history, and kept it open across next-round setup transitions until explicitly dismissed.

- [x] Add inactivity-based automatic logout for authenticated users.
- [x] Expose the current room list in the multiplayer UI for non-admin players.
- [x] Show all captured cards on the board with a compact overlapping layout instead of truncating to a few cards.
- [x] Add test bots that can join a room and take simple automated actions for multiplayer testing.
- [x] Expand synced score details and round history so players can inspect full captured-card and settlement breakdowns.
- [x] Verify each touched file with `git diff -- <file>`.
- [x] Run the relevant tests and build commands for this multiplayer UX/automation slice.

Review:
- Added client-side idle logout, server-backed public room list refresh, host-only test bot controls, overlapped captured-card stacks, and richer synced score/history panels.
- Added server tests for public room summaries, idle-room test bots, and persisted history detail fields.
